...
interface TradeStore {
  ...
  importPredefinedTags: (tags: string[]) => void;
  importPredefinedSetups: (setups: string[]) => void;
}
...
export const useTradeStore = create<TradeStore>((set) => ({
  ...
  importPredefinedTags: (tags) => {
    set((state) => ({
      predefinedTags: Array.from(new Set([...state.predefinedTags, ...tags])),
    }));
  },
  importPredefinedSetups: (setups) => {
    set((state) => ({
      predefinedSetups: Array.from(new Set([...state.predefinedSetups, ...setups])),
    }));
  },
}));
