...
export function Analytics() {
  ...
  const calculateMetrics = (trades: Trade[], groupBy: keyof Trade | 'tags' | 'dayOfWeek'): ProfitabilityMetric[] => {
    const metrics = trades.reduce((acc, trade) => {
      ...
      if (groupBy === 'tags') {
        ...
        values = [format(new Date(trade.exitDate), 'EEEE')];
      } else {
        values = [trade[groupBy] as string];
      }
  ...
  const mostProfitableDays = calculateMetrics(filteredTrades, 'dayOfWeek');
  ...
  const calculateCombinations = (trades: Trade[]): CombinedMetric[] => {
    const combinations = trades.reduce((acc, trade) => {
      const dayOfWeek = format(new Date(trade.exitDate), 'EEEE');
      trade.tags.forEach(tag => {
        const key = `${trade.instrument}|${trade.direction}|${trade.setup}|${tag}|${dayOfWeek}`;
        if (!acc[key]) {
          acc[key] = {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {mostProfitableDays[0] && (
              <MetricCard title="Top Day" metric={mostProfitableDays[0]} />
            )}
          </div>
        </div>
          ...
          {profitableCombinations[0] && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Day of Week</p>
                  <p className="text-lg font-semibold text-gray-900 dark:text-white">
                    {profitableCombinations[0].dayOfWeek}
                  </p>
                </div>
              </div>
              <div>
